                        ...
                        has_joint_states_(false)
    {
        ...
        if (param_client->wait_for_service(std::chrono::seconds(2))) {
            try {
                ...
                RCLCPP_WARN(this->get_logger(), "Failed to get robot_description from parameter: %s", e.what());
...
private:
    ...
    void setupJointLimits(const urdf::Model& /*model*/)
    {
        ...
        if (right_num_joints != right_min_joint_positions_.size()) {
            RCLCPP_WARN(this->get_logger(), "Right arm joint count mismatch: chain has %d joints, hardcoded limits for %zu",
        ...
        if (left_num_joints != left_min_joint_positions_.size()) {
            RCLCPP_WARN(this->get_logger(), "Left arm joint count mismatch: chain has %d joints, hardcoded limits for %zu",
    ...
    void jointStateCallback(const sensor_msgs::msg::JointState::SharedPtr msg)
    {
        ...
        for (size_t i = 0; i < right_joint_names_.size(); i++) {
            ...
            if (it != msg->name.end()) {
                ...
                RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 5000,
                                    "Joint %s not found in joint_states", right_joint_names_[i].c_str());
            }
        }
        ...
        for (size_t i = 0; i < left_joint_names_.size(); i++) {
            ...
            if (it != msg->name.end()) {
                ...
                RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 5000,
                                    "Joint %s not found in joint_states", left_joint_names_[i].c_str());
            }
        }
    ...
    void checkCurrentJointLimits()
    {
        ...
        if (right_all_within_limits && left_all_within_limits) {
            ...
            RCLCPP_WARN(this->get_logger(), "⚠️ Some joints are outside limits - this is OK for initialization");
        }
    }
    ...
    void rightTargetPoseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
    {
        ...
        // VR Publisher에서 이미 모든 좌표 변환을 완료하고 base_link frame으로 보냄
        // 여기서는 base_link → arm_base_link 변환만 수행
    ...
    }    void solveIK(const geometry_msgs::msg::PoseStamped& target_pose, const std::string& arm)
    {
        ...
        if (ik_result >= 0) {
            ...
            if (!all_within_limits) {
                RCLCPP_ERROR(this->get_logger(), "❌ %s arm IK solution violates joint limits! Skipping.", arm.c_str());
                return;
            }
            ...
            RCLCPP_ERROR(this->get_logger(), "❌ %s arm IK failed: %s (code: %d)", arm.c_str(), error_msg.c_str(), ik_result);
