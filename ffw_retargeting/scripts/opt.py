...
class DexPilotOptimizer:
    ...
    def __init__(
        ...
        # Ensure these are in the FK computation cache
    ...
    def get_objective_function(self, target_vector: np.ndarray, last_qpos: np.ndarray, target_dir: np.ndarray = None):
        ...
        def loss_fn(qpos):
            ...
            # --- New Orientation Loss ---
            if target_dir is not None:
                # Calculate vector from proximal link to tip link for each finger
                r_prox = body[self.proximal_indices, :]
                r_tip = body[self.tip_indices, :]
                r_dir = r_tip - r_prox
                
                # Normalize robot direction vectors
                # autograd's norm might not support keepdims in vjp, so we reshape manually
                r_dir_norm_val = anp.linalg.norm(r_dir, axis=1)
                r_dir_norm = r_dir / (anp.reshape(r_dir_norm_val, (-1, 1)) + 1e-6)
                
                # Human target_dir is already normalized in retarget.py
                # Minimize (1 - cosine_similarity)
                cos_sim = anp.sum(r_dir_norm * anp.array(target_dir), axis=1)
                dir_loss = anp.sum(1.0 - cos_sim) * self.orientation_weight
            else:
                dir_loss = 0.0

            reg_loss = self.norm_delta * anp.sum((qpos - last_qpos) ** 2)
            return pos_loss + dir_loss + reg_loss
